...
Specifically, Flycheck-Annotate mode is enabled in all buffers where `(lambda nil (when
...
Specifically, Flycheck-Lsp mode is enabled in all buffers where `(lambda nil (when
(flycheck-lsp--available-command major-mode) (flycheck-lsp-mode 1)))' would do it.
...
Specifically, Flycheck-Eglot mode is enabled in all buffers where `(lambda nil (when

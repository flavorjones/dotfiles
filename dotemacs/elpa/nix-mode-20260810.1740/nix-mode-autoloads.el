...
In addition to any hooks its parent mode `json-ts-mode' might have run, this mode runs the hook
`nix-drv-mode-hook', as the final or penultimate step during initialization.
...
"Toggle Nix-Prettify mode in many buffers.
Specifically, Nix-Prettify mode is enabled in all buffers where `nix-prettify-turn-on' would do it.
